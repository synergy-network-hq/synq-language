    ...
    fn gen_statement(&mut self, stmt: &Statement) -> Result<(), String> {
        match stmt {
            Statement::VariableDeclaration(name, _ty, expr) => {
                ...
                // Store variable (simplified - would need proper variable storage)
            }
            ...
            Statement::Require(expr, _msg) => {
                ...
                let error_label = format!("{}_require_error", self.current_function.as_ref().unwrap_or(&"global".to_string()));
                ...
                self.label_positions.insert(error_label, self.assembler.code_len());
            ...
            Statement::If(cond, then_block, else_block) => {
                ...
                let else_label = format!("{}_if_else_{}", 
                    self.current_function.as_ref().unwrap_or(&"global".to_string()),
                    self.assembler.code_len());
                let end_label = format!("{}_if_end_{}", 
                    self.current_function.as_ref().unwrap_or(&"global".to_string()),
                    self.assembler.code_len());
                ...
                if else_block.is_some() {
                    ...
                    self.label_positions.insert(else_label, self.assembler.code_len());
                    ...
                    self.label_positions.insert(end_label, self.assembler.code_len());
                } else {
            ...
            Statement::RequirePqc(pqc_block, fallback) => {
                ...
                let success_label = format!("{}_require_pqc_success_{}", 
                    self.current_function.as_ref().unwrap_or(&"global".to_string()),
                    self.assembler.code_len());
                let failure_label = format!("{}_require_pqc_failure_{}", 
                    self.current_function.as_ref().unwrap_or(&"global".to_string()),
                    self.assembler.code_len());
                let end_label = format!("{}_require_pqc_end_{}", 
                    self.current_function.as_ref().unwrap_or(&"global".to_string()),
                    self.assembler.code_len());
                ...
                self.label_positions.insert(success_label.clone(), self.assembler.code_len());
                self.assembler.emit_op(OpCode::Jump);
                let patch_pos = self.assembler.code_len();
                self.assembler.emit_u32(0);
                self.jump_patches.push((patch_pos, end_label.clone()));
                ...
                self.label_positions.insert(failure_label, self.assembler.code_len());
            ...
            _ => {
                return Err(format!("Unsupported statement type in code generation"));
            }
        }
        Ok(())
    }
        ...
        match expr {
            ...
            Expression::Call(name, args) => {
                ...
                if PqcIntegration::is_pqc_function(name) {
                    if name.starts_with("verify_mldsa") {
                        self.assembler.emit_op(OpCode::MLDSAVerify);
                    } else if name.starts_with("verify_fndsa") {
                        self.assembler.emit_op(OpCode::FNDSAVerify);
                    } else if name.starts_with("verify_slhdsa") {
                        self.assembler.emit_op(OpCode::SLHDSAVerify);
                    } else if name.starts_with("mlkem_") {
            ...
            Expression::Ternary(cond, then_expr, else_expr) => {
                ...
                let else_label = format!("{}_ternary_else_{}", 
                    self.current_function.as_ref().unwrap_or(&"global".to_string()),
                    self.assembler.code_len());
                let end_label = format!("{}_ternary_end_{}", 
                    self.current_function.as_ref().unwrap_or(&"global".to_string()),
                    self.assembler.code_len());
